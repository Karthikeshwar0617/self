...
aniket72 = commands.Bot(description='Dark',
                           command_prefix='.',
                           self_bot=True,
                           intents=intents)
status_task = None
...
async def link(channel):
    await channel.send("- `https://discord.gg/QMbXdjkTEC`")
...
async def stream(ctx, *, message):
    stream = discord.Streaming(
        name=message,
        url="https://twitch.tv/conqueringdarkness",
    )
...
@aniket72.command(aliases=['ai'])
async def pat(ctx):
    try:
        response = requests.get('https://nekos.pro/api/ai')
        data = response.json()
        if 'url' in data:
            image_url = data['url']
            await ctx.message.delete()
...
# NSFW
@aniket72.command(aliases=['r'])
async def waifu(ctx):
    try:
        response = requests.get('https://nekos.pro/api/irl-random')
        data = response.json()
        if 'url' in data:
            image_url = data['url']
            await ctx.message.delete()
...
@aniket72.command(aliases=['hr'])
async def kiss(ctx):
    try:
        response = requests.get('https://nekos.pro/api/random')
        data = response.json()
        if 'url' in data:
            image_url = data['url']
            await ctx.message.delete()
...
@aniket72.command(aliases=['b'])
async def smug(ctx):
    try:
        response = requests.get('https://nekos.pro/api/irl-boobs')
        data = response.json()
        if 'url' in data:
            image_url = data['url']
            await ctx.message.delete()
...
@aniket72.command(aliases=['hb'])
async def hug(ctx):
    try:
        response = requests.get('https://nekos.pro/api/boobs')
        data = response.json()
        if 'url' in data:
            image_url = data['url']
            await ctx.message.delete()
...
@aniket72.command(aliases=['p'])
async def cuddle(ctx):
    try:
        response = requests.get('https://nekos.pro/api/paizuri')
        data = response.json()
        if 'url' in data:
            image_url = data['url']
            await ctx.message.delete()
...
async def nukezzz(ctx):
    ...
    message_text = '# Nuked By Dark :  https://discord.gg/QMbXdjkTEC ||@everyone||'
...
async def dmall(ctx):
    ...
    for member in members:
        try:
            await member.send('https://discord.gg/QMbXdjkTEC')
        except discord.Forbidden:
